...
public class LoadTruckPanel2 extends JPanel {
	...
	/**
	 * Panel for loading a Truck with parcels (used by truck driver).
	 */
	public LoadTruckPanel2() {
		...
		if(Session.originWarehouse!=null) {
			for(int i=0;i<widArr2.length;i++) {
				if(widArr2[i].equals(Session.originWarehouse.getWarehouseId())) {
					comboBoxWhId.setSelectedIndex(i);
					comboBoxWhId.setEnabled(false);
				}
			}
		}
		add(comboBoxWhId);
		...
		if(Session.destWarehouse!=null) {
			for(int i=0;i<widArr4.length;i++) {
				if(widArr4[i].equals(Session.destWarehouse.getWarehouseId())) {
					comboBoxWhId2.setSelectedIndex(i);
					comboBoxWhId2.setEnabled(false);
				}
			}
		}
		add(comboBoxWhId2);
...
//						switchPanels(Session.truckDriverPanel);
						switchPanels(new SendTruckPanel());
					}
				}
