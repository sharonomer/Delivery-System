...
public class AddLargeParcelPanel extends JPanel {
	...
	/**
	 * Panel for creating a new Large parcel.
	 */
	public AddLargeParcelPanel() {
		setBackground(new Color(25, 25, 112));		
		setLayout(null);
		this.setBounds(0, 0, 464, 411);
		
		JLabel lblNewUserInfo = new JLabel("New Large Parcel:");
		...
		btnSignUp_1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(vin.length()!=0)
					AddLargeParcelPanel.this.vin=vin;
				else {
				...
				if(!noReceivers.isVisible()&&!lblCantBeEmpty.isVisible())
					try{
						if(sys.getAllParcelsMap().containsKey(vin)){
							...
							sys.addLargeParcel(signUpId.getText(), (long) comboBox2.getSelectedItem());
