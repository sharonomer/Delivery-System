...
public class AddCoordinatorPanel extends JPanel {
	...
	/**
	 * Panel for adding a new Co-ordinator.
	 */
	public AddCoordinatorPanel() {
		...
		JLabel lblNewUserInfo = new JLabel("New Co-ordinator:");
...
//				badEmail.setVisible(false);
				...
				if(isOnlyDigit(id)&&id.length()!=0&&!id.equals("Admin"))
					AddCoordinatorPanel.this.id=Long.parseLong(id);
				else {
					if(id.length()==0) {
						onlyNumsSU.setVisible(false);
...
//						lblUserAddedSuccessfully.setVisible(false);
				...
				if(p.length()!=0)
					AddCoordinatorPanel.this.password=p;
				else {
...
//							lblUserAddedSuccessfully.setVisible(false);
							...
							sys.addCoordinator(Long.parseLong(id), lblFirstName.getText(), lblLastName.getText(),
									(Date)datePicker.getModel().getValue(), address);
							sys.addLoginInfo(id, p);
...
//				lblUserAddedSuccessfully.setVisible(false);
		...
		JLabel coordinatorIcon = new JLabel(" ");
		coordinatorIcon.setBounds(228, 160, 226, 217);
//		coordinatorIcon.setIcon(new ImageIcon("img/coordinator_icon.png"));
		coordinatorIcon.setIcon(new ImageIcon(getClass().getClassLoader().getResource("coordinator_icon.png")));
		add(coordinatorIcon);
	}
